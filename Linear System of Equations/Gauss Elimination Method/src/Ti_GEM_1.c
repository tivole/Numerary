...
void ZeroingTheCornerOfMatrix(double **A, int corner, int h, int n) {
  ...
  for(i = 0; i < n; i++) {
    for(j = 0; j < n; j++) {
      if((i <= (h - 1 - j)) && (corner == 1))  A[i][j] = 0;
      else if((j >= (n - h + i)) && (corner == 2))  A[i][j] = 0;
      else if((i >= (n - h + j)) && (corner == 3))  A[i][j] = 0;
      else if(((i + j) >= (2*n - h - 1)) && (corner == 4))  A[i][j] = 0;
    }
  }
  return;
}
